...
router.get("/", async (req, res) => {
  try {
    ...
    const categories = await Category.find({}).sort({ title: 1 });  // ✅ fetch categories

    res.render("shop/home", { pageName: "Home", products, categories });  // ✅ send to view
  } catch (error) {
    console.log(error);
    res.redirect("/");
  }
...
router.get('/cart', function (req, res) {
  if (!req.session.cart) {
    return res.render('cart', {
      ...
      pageName: 'Cart',
      csrfToken: req.csrfToken() // ✅ Fix added here
    });
  }
  res.render('cart', {
    cart: req.session.cart,
